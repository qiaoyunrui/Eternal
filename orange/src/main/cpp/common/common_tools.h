...
#ifndef ETERNAL_COMMONTOOLS_H
#define ETERNAL_COMMONTOOLS_H
...
inline int readShortFromFile(short *shortArray, int size, FILE *fp) {
    int actualSize = static_cast<int>(fread(shortArray, 2, static_cast<size_t>(size), fp));
    LOGI("read data expected size is %d and size is %d", size, actualSize);
    if (actualSize != size) {
        LOGI("actualSize is %d size is %d", actualSize, size);
        int flag = feof(fp);
        if (flag != 0 && actualSize == 0) {
            //到达文件末尾
            LOGI("end of file, actualSize is %d", actualSize);
            return -1;
        }
    }
    return actualSize;
...
inline int readByteFromFile(byte *byteArray, int size, FILE *fp) {
    int actualSize = static_cast<int>(fread(byteArray, 1, static_cast<size_t>(size), fp));
    //	LOGI("read data expected size is %d and size is %d", size, actualSize);
    if (actualSize != size) {
        int flag = feof(fp);
        if (flag != 0 && actualSize == 0) {
                                ...
                                INT16_MIN) : (a > 0 &&
                                              ...
                                              (((int) a *
                                               ...
                                               INT16_MAX)
                                                    :
                                              a + b);
    return static_cast<SInt16>(tmp > INT16_MAX ? INT16_MAX : (tmp < INT16_MIN ? INT16_MIN : tmp));
}

//合并两个float，返回一个short
inline SInt16 TPMixSamplesFloat(float a, float b) {
    int tmp = static_cast<int>(a < 0 && b < 0 ? ((int) a + (int) b) -
                                                (((int) a * (int) b) / INT16_MIN) :
                               (a > 0 &&
                                b > 0 ?
                                ((int) a +
                                 (int) b) -
...
inline SInt16 convertShort(byte *bytes) {
    return static_cast<SInt16>((bytes[0] << 8) + (bytes[1] & 0xFF));
}
...
inline SInt16 adjustAudioVolume(SInt16 source, float volume) {
    ...
    **/
    ...
    if (absTemp < 29490) {
        result = static_cast<SInt16>(temp);
    } else if (absTemp > 46285) {
        result = static_cast<SInt16>(32767 * sign);
    } else if (absTemp > 32630) {
        result = static_cast<SInt16>(((absTemp - 29490) / 8 + 30668) * sign);
    } else {
        result = static_cast<SInt16>(((absTemp - 29490) / 2 + 29490) * sign);
    }
    return result;
}
